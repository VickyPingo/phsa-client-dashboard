...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${
      ...
      <span className={`w-1.5 h-1.5 rounded-full mr-1 ${isClosed ? 'bg-slate-400' : 'bg-emerald-500'}`} />
      {status}
    </span>
  );
}

function MarisNoteCell({ client, onUpdate }: {
  client: Client;
      ...
      <div className="flex gap-1.5 flex-wrap">
        {NOTE_COLOURS.map(c => (
          <button key={c.value} onClick={() => setColour(c.value)}
            className={`w-6 h-6 rounded-full border-2 transition-transform hover:scale-110 ${colour === c.value ? 'border-slate-500 scale-110' : 'border-slate-200'}`}
            style={{ backgroundColor: c.value }} title={c.label}
          />
        ))}
      </div>
      <textarea
        className="w-full text-xs border border-slate-200 rounded-lg p-2 resize-none focus:outline-none focus:ring-2 focus:ring-primary-400"
        rows={5} value={note} onChange={e => setNote(e.target.value)}
        placeholder="Add a note..." autoFocus style={{ backgroundColor: colour }}
      />
      <div className="flex gap-2 justify-end">
        <button onClick={e => { e.stopPropagation(); handleClose(); }} className="text-xs px-2 py-1 text-slate-500 hover:text-slate-700 rounded">Cancel</button>
        <button onClick={handleSave} disabled={saving} className="text-xs px-3 py-1.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:opacity-50 font-medium">
          {saving ? 'Saving...' : 'Save'}
        </button>
      </div>
    </div>,
  ...
  return (
    <>
      ...
      >
        {hasNote ? <span className="text-slate-700">{client.maris_note}</span> : <span className="text-slate-400 italic">+ note</span>}
      </div>
      {popover}
    </>
  );
...
export default function ClientsTable({ clients, loading, totalCount, page, pageCount, onPageChange, onRowClick, onNoteUpdate }: Props) {
  ...
  const columns = [
    ...
    col.display({
      ...
      header: "Mari's Notes",
      enableSorting: false,
      cell: ({ row }) => <MarisNoteCell client={row.original} onUpdate={onNoteUpdate} />,
    }),
  ];

  const table = useReactTable({
    data: clients, columns,
    state: { sorting },
    ...
    <div className="space-y-3">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={columns.length} className="text-center py-12 text-slate-400 text-sm">No clients found</td></tr>
              ) : (
                table.getRowModel().rows.map((row, i) => (
                  <tr key={row.id} onClick={() => onRowClick(row.original)}
                    className={`border-b border-slate-50 cursor-pointer transition-colors hover:bg-primary-50/50 ${i % 2 === 0 ? '' : 'bg-slate-50/30'}`}
                  >
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-slate-500">
          {totalCount === 0 ? 'No results' : `${from}–${to} of ${totalCount} clients`}
        </p>
        <div className="flex items-center gap-1">
          <button onClick={() => onPageChange(page - 1)} disabled={page === 0 || loading} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 disabled:opacity-30 transition-colors">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <span className="text-xs text-slate-500 px-1">{pageCount === 0 ? '—' : `${page + 1} / ${pageCount}`}</span>
          <button onClick={() => onPageChange(page + 1)} disabled={page >= pageCount - 1 || loading} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 disabled:opacity-30 transition-colors">
